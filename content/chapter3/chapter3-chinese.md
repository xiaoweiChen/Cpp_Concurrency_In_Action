...
幸运的是，这段问题代码还没有做什么出格的事情：因为代码已标记所有可访问的数据结构是互斥的。在这种情况下，在**foo()**中调用**unprotected->do_something()**应该是被忘记了。但不幸的是，这种情况下，C++线程库无法提供帮助；它只能在程序中，用正确的互斥锁去保护你的数据。从乐观的角度上看，在这种情况下，你是有方法可循的：切勿在互斥锁范围外传递保护数据的指针或引用，无论是函数返回值，还是存储在外部可见内存，亦或是将他们已参数的形式传递到用户提供的函数中去。
