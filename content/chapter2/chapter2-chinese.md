...
线程库不会限制你去检查线程标识是否一样；`std::thread::id`类型对象提供相当丰富的对比操作，比如，提供为不同的值进行排序。这就意味着允许程序员将其当做为容器的键值，或做排序，亦或做以任何一种方式进行比较。比较操作将不同值的`std::thread::id`按默认顺序排序，所以这个行为是直观，且可预见的：当 a\<b，b\<c时，得a\<c，等等。标准库也提供`std::hash<std::thread::id>`容器，这样`std::thread::id`类型可以作为无序容器的键值了。
