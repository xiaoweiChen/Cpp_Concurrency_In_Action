...
好的！看来你已经决定在你的应用中使用并发和多线程了。现在做点什么呢？如何去启动线程？如何知道它们已经结束了？还是，如何持续监视它们？C++标准库让线程管理变得相对容易很多，你将在后面看到，只需要管理`std::thread`对象中关联的线程即可。不过，标准库的灵活性，会让这些任务不会像想象的那么简单。
在这一章，我将从一些基本的东西开始说起：启动一个线程，等待其结束，或让它在后台运行。再看看怎么给已经启动的线程函数传递参数，以及怎么将一个线程的所有权从一个`std::thread`对象移交给另一个。最后，我们来看看怎么确定线程数，以及识别特殊的线程。

##2.1 线程管理基础

每个C++程序至少有一个线程：线程执行**main()**函数。程序中其他线程可以使用其他的入口函数。之后，这些线程与原始线程（以**main()**作为入口函数的线程）同时运行。如同程序在**mian()**函数执行完会退出一样，当线程中的入口函数执行完成后，线程也会退出。如你将看到的那样，当你为一个线程创建了一个`std::thread`对象，现在你需要等待这个线程结束；但前提是，你得启动它。下面我们就来启动线程。
...
我们经常需要线程去细化一个算法的总体工作，并且在算法结束的之前，所有的线程必须要结束。在清单2.7中的简单实现，意味着线程所做的工作都是独立的，并且结果仅会受到共享数据的影响。如果**f()**有返回值，那么这个返回值就依赖于线程得到的结果，然后在写入返回值之前，会检查使用共享数据的线程是否终止。操作结果在不同线程中的转移的其他替代方案，我们会在第四章中再次讨论。
将`std::thread`放入`std::vector`是向线程自动化管理迈出的第一步：并非为这些线程创建独立的变量，并且将他们直接加入，可以把它们当做一个组。创建一组线程（数量在运行时确定），可以使得你的这一步，迈的更大些；而非像清单2.7那样创建固定数量的线程。
...
启动线程数量是，计算量的最大值和硬件支持线程数中较小的那个 ③ 。因为上下文平凡的切换会降低线程的性能，所以你肯定不想启动的线程数，多于硬件支持的线程数量（称为**超额认购**（*oversubscription*））。当`std::thread::hardware_concurrency()`返回0，你可以选择一个合适的数作为你的选择；在本例中我选择了“2”这个数。你肯定也不像在一台单核机器上启动太多的线程，这样反而会降低性能，在这种情况下，你最终会放弃使用并发。
每个线程中处理的元素数量是，范围中元素的总量除以线程的个数得出的 ④ 。这里有可能分配不均，在后面你可以对其进行处理。
现在，你知道你要运行多少个线程了，并且你可以创建一个`std::vector<T>`容器存放中间结果，并且为线程创建一个`std::vector<std::thread>`容器 ⑤。这里需要注意一下，你要启动的线程必须比num_threads, 因为在启动之前你已经有一个线程了（主线程）。
...
当你已经累加最终块中的结果后，你可以等待`std::for_each` ⑩ 创建的线程
（如同在清单2.7中做的那样），之后使用`std::accumulate`将所有结果进行累加 ⑪。
在结束这个例子之前，这里需要指出的是T的类型与加法操作是没有什么关联的（比如，T可以是float或double），**parallel_accumulate**所得到的结果可能与`std::accumulate`得到的结果有所不同，这是因为对范围中元素的分组所致。同样的，这里对迭代器的要求更加严格：它们必须都是向前迭代器（*forward iterator*），而`std::accumulate`可以在单传输入迭代器（*input iterators*）的情况下工作，并且T必须是可以默认构造的，这样才能保证你能创建出results容器。对于算法并行来说，这些变化是普遍存在的；
不过，由于算法本身的特性，要使它们并行需要使用不用的方式。算法并行会在第8章有更加深入的讨论。还需要注意的是，因为你不能直接从一个线程中返回一个值，所以你需要传递results容器的引用到线程中去。另一个办法是，通过地址来获取线程执行的结果，在第4章中，我们将使用期望（*futures*）来完成这一方案。
在这种情况下，当线程运行时，所有必要的信息都需要传入到线程中去，包括存储计算结果的位置。不过，并非总是如此：有时候这是一种识别线程的必要方式。你可以传递一个标识数，例如，在清单2.7中的i，但当需要标识的函数在调用栈的深层，并可让任何线程进行调用，那么使用标识数的办法在这里就不能用了。不过，在设计及C++的线程库时，我们就有预见到了这种情况，之后的实现中就给每个线程附加了唯一的标识符。

##2.5 识别线程

线程标识类型是`std::thread::type`，可以通过两种方式检索到它。第一种，可以通过调用` std::thread`对象的成员函数`get_id()`来直接获取。如果`std::thread`对象没有与任何执行线程相关联，那么`get_id()`将返回`std::thread::type`默认构造值，这个值的意思是“没有任何线程”。第二种，再当前线程中，调用`std::this_thread::get_id()`（这个函数定义在<thread>头文件中）也可以获得线程标识。
`std::thread::id`对象可以自由的拷贝和对比；这样就不会使用更多的标识符。如果两个对象的`std::thread::id`相等，它们就是同一个线程，或者都“没有任何线程”。如果不等，那么它们就代表了两个不同线程，或者一个有相关线程，另一没有。
线程库不会限制你去检查线程标识是否一样；`std::thread::id`类型对象提供相当丰富的对比操作，比如，提供为不同的值进行排序。这就意味着允许程序员将其当做为容器的键值，或做排序，亦或做以任何一种方式进行比较。比较操作将不同值的`std::thread::id`按默认顺序排序，所以这个行为是直观，且可预见的：当```a<b```，```b<c```时，得```a<c```，等等。标准库也提供`std::hash<std::thread::id>`容器，这样`std::thread::id`类型可以作为无序容器的键值了。
` std::thread::id`实例都常用来检查线程是否需要进行一些操作。比如：当用线程来分割一项工作（如清单2.8），初始线程可能要做一些与其他线程不同的工作。在这种情况下，在启动其他线程前，它可以将自己的线程ID通过`std::this_thread::get_id()`得到，并进行存储。然后，就是算法的核心部分（所有线程都一样的）：每个线程都要检查一下，其拥有的线程ID是否与初始线程的ID相同。
...
```
另外，当前线程的`std::thread::id`将存储到一个数据结构中。之后会在这个结构提中对当前线程的ID与存储的线程ID做对比，来决定操作是做“允许”操作，还是“需要”操作（*permitted/required*）。
同样的，线程ID可以在容器中作为键值，作为线程和本地存储不适配的替代方案。例如，容器可以由一个控制线程存储每个在其控制下线程的信息，或在多个线程中互传信息。
`std::thread::id`足够作为一个线程的通用标识符；当标识符只与语义相关（比如，一个数组的索引），这时就需要其他方案来解决了。你甚至可以使用输出流（`std::cout`）来记录一个`std::thread::id`对象的值。

```c++
std::cout<<std::this_thread::get_id();
```

输出的精确程度是依赖于实现的；这只能保证线程ID与打印出来的值相等，不同的输出代表着不同线程。这对于记录和调试来说，是很有帮助的。但是，打印出来的值没有任何语义，所以除了给记录和调试带来一些帮助之外，就没有太大的用处了。

##2.6 总结

在这章中讨论了C++标准库中基本的线程管理方式：启动线程，等待他们结束，并且不等待他们结束（因为需要它们运行在后台）。你了解应该如何在线程启动前，向线程函数中传递参数，如何转移线程的所有权，如何使用线程组来分割任务。最后，讨论了使用线程表示来确定与之关联数据，和特殊线程的特殊解决方案。虽然，你现在已经可以纯粹的依赖线程，使用独立的数据，做独立的任务（如同清单2.8），但有时线程运行时，确实需要有共享数据。第3章会讨论共享数据和线程的直接关系。第4章会讨论，在（有/没有）共享数据情况下的线程同步操作。
